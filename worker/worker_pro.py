    ...
    for par in COINS:
        ...
        if mark is None:
            ...
            items.append({
                ...
                "atual": 0.0, "alvo": float(atual), "ganho_pct": 0.0, "assert_pct": 0.0,
        ...
        if (not o1) or (not o4):
            ...
            items.append({
                "par": par, "side": "NÃO ENTRAR",
                "atual": float(mark), "alvo": float(atual), "ganho_pct": 0.0, "assert_pct": 0.0,
                "prazo": "", "zona": "", "risco": "", "prioridade": "",
                "data": data, "hora": hora, "price_source": src
            })
            continue
    ...
    if g < float(GAIN_MIN_PCT) or a < float(ASSERT_MIN_PCT):
        ...
        if sig.side == "NÃO ENTRAR":
            items.append({
                ...
                "alvo": float(atual), "ganho_pct": 0.0, "assert_pct": 0.0,
                # ✅ regra oficial: vazios
                "prazo": "", "zona": "", "risco": "", "prioridade": "",
                "data": data, "hora": hora, "price_source": src
            })
